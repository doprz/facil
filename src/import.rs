...
const KNOWN_TOP_LEVEL_KEYS: &[&str] =
    &["name", "root", "project_root", "socket_name", "tmux_options", "pre", "post", "pre_window", "windows", "project_hooks"];
...
pub fn convert(yaml: &str) -> Result<(Project, Vec<String>), ConfigError> {
    ...
    let root_dir = root
        ...
        .and_then(Value::as_str)
        .map(str::to_string);
    let socket_name = root.get("socket_name").and_then(Value::as_str).map(str::to_string);
    let tmux_options = root.get("tmux_options").and_then(Value::as_str).map(str::to_string);
    ...
    if let Some(hooks) = root.get("project_hooks").and_then(Value::as_mapping) {
        if let Some(v) = hooks.get("on_project_start") {
            ...
            warnings.push("project_hooks.on_project_start has no direct facil equivalent; appended to `pre`".to_string());
        }
        for key in ["on_project_exit", "on_project_stop"] {
            if let Some(v) = hooks.get(key) {
                post.extend(as_str_list(Some(v)));
                warnings.push(format!("project_hooks.{key} has no direct facil equivalent; appended to `post`"));
            }
        }
        for key in ["on_project_first_start", "on_project_restart"] {
            if hooks.get(key).is_some() {
                warnings.push(format!("project_hooks.{key} is not supported by facil and was skipped"));
            }
        }
    }
    ...
    for key in root.keys() {
        ...
        {
            warnings.push(format!("top-level `{key_str}` is not supported by facil and was skipped"));
        }
    }
    ...
    let project = Project { name, root: root_dir, pre, post, tmux_options, socket_name, windows };
    Ok((project, warnings))
}

fn convert_window(entry: &Value, top_level_pre_window: &[String], index: usize) -> Result<(Window, Vec<String>), ConfigError> {
    let mapping = entry
        .as_mapping()
        .ok_or_else(|| ConfigError::ImportField(format!("windows[{index}] must be a single-key mapping")))?;
    ...
    let window = match value {
        Value::String(cmd) => Window {
            ...
            panes: vec![Pane { commands: vec![cmd.clone()], root: None }],
        },
        Value::Null => Window {
            name,
            layout: None,
        ...
        _ => return Err(ConfigError::ImportField(format!("windows[{index}] has an unsupported value shape"))),
    };

    Ok((window, warnings))
}

fn convert_window_mapping(name: String, m: &Mapping, top_level_pre_window: &[String], warnings: &mut Vec<String>) -> Window {
    ...
    let panes = match m.get("panes") {
        None => vec![Pane { commands: vec![], root: window_root.clone() }],
        Some(Value::Sequence(seq)) => {
            seq.iter().map(|p| convert_pane(p, window_root.as_deref(), &name, warnings)).collect()
        }
        Some(_) => {
            warnings.push(format!("windows.{name}.panes has an unsupported shape and was imported as a single empty pane"));
            vec![Pane { commands: vec![], root: window_root.clone() }]
        }
    };
    ...
    for key in m.keys() {
        ...
        {
            warnings.push(format!("windows.{name}.{key_str} is not supported by facil and was skipped"));
        }
    }

    Window { name, layout, pre_window, panes }
}

fn convert_pane(value: &Value, window_root: Option<&str>, window_name: &str, warnings: &mut Vec<String>) -> Pane {
    match value {
        Value::String(s) => Pane { commands: vec![s.clone()], root: window_root.map(str::to_string) },
        Value::Null => Pane { commands: vec![], root: window_root.map(str::to_string) },
        Value::Sequence(seq) => Pane {
            commands: seq.iter().filter_map(|v| v.as_str().map(str::to_string)).collect(),
            root: window_root.map(str::to_string),
        },
        _ => {
            warnings.push(format!("a pane in window `{window_name}` has an unsupported shape and was imported empty"));
            Pane { commands: vec![], root: window_root.map(str::to_string) }
        }
    }
}
...
"#;
    ...
    fn converts_representative_config() {
        ...
        assert_eq!(editor.panes[1].commands, vec!["cargo watch -x test", "echo ready"]);
        ...
        assert!(warnings.iter().any(|w| w.contains("facil's `pre` runs after")));
    }
